...
# 팀 로고 이미지 경로 설정
team_logo_path = "team_logo2/"  # team_logo2 디렉토리 경로로 수정
...
# 뉴스 데이터 크롤링 함수 정의
def fetch_news_for_team(team_code):
    ...
    for item in news_data:
        ...
        driver.back()
        time.sleep(1)  # 페이지 로딩 시간을 줄이기 위해 sleep 시간을 줄임

    driver.quit()
    return team_code, news_details

# 병렬 처리로 모든 팀의 뉴스를 가져오기
def load_all_news_data(team_codes):
    news_results = {}
    with ThreadPoolExecutor(max_workers=len(team_codes)) as executor:
        futures = {executor.submit(fetch_news_for_team, team_code): team_code for team_code in team_codes.values()}
...
# 뉴스 데이터 로드
news_data = load_all_news_data(team_codes)
