        ...
        with button_col:
            if st.button("조회", key=f"button_{team_code}"):
                with st.spinner("뉴스를 로드하는 중입니다..."):
                    ...
                    # 워드 클라우드 이미지 저장
                    st.session_state[team_code] = wordcloud.to_array()  # 세션 상태에 워드클라우드 저장

        # 저장된 워드클라우드 표시
        if team_code in st.session_state and st.session_state[team_code] is not None:
            st.image(st.session_state[team_code], caption=f"{team_name} 관련 뉴스 워드 클라우드", use_column_width=True) #width=nn(특정크기로 조절가능)
